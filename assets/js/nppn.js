  ...
  function initDirectory(onUseItem) {
    ...
    const render = () => {
      ...
      if (!shown.length) {
        body.innerHTML = `
          <tr>
            <td class="nppn-directory-empty" colspan="6">
        ...
        body.innerHTML = shown
          .map(
            (item) => `
            <tr>
              <td>${escapeHtml(item.klu)}</td>
              <td>${escapeHtml(item.uraian)}</td>
              ${rateCell(item.r10, "10 ibu kota")}
              ${rateCell(item.rprov, "Ibu kota provinsi")}
              ${rateCell(item.rlain, "Daerah lainnya")}
              <td>
                <button type="button" class="nppn-use-button" data-nppn-no="${item.no}">
                  Gunakan
                </button>
              </td>
            </tr>`
