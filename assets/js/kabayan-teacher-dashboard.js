(() => {
  ...
  const el = {
    ...
    newClassCode: document.getElementById("newClassCodeTeacher"),
  ...
  async function loadDashboard() {
    ...
    try {
      const [attemptRows, classRows, participantRows] = await Promise.all([
        STORE.fetchAttempts(),
        STORE.fetchClasses(),
        STORE.fetchRegisteredParticipants()
      ]);

      allAttempts = attemptRows;
      classes = classRows;
      registeredParticipants = participantRows;
  ...
  function classParticipantCount(classId) {
    return registeredParticipants.filter(p => p.class_id === classId).length;
  }

  function classAttemptCount(classId) {
    return allAttempts.filter(a => a.class_id === classId).length;
  }

  function shareClassUrl(classCode) {
    return `${location.origin}/evaluasi-pph21.html?class=${encodeURIComponent(classCode)}`;
  }
  ...
  function renderClassManagement() {
    ...
    if (!classes.length) {
      el.classList.innerHTML = `
        <div class="class-empty">
          Belum ada kelas. Buat kelas pertama menggunakan formulir di atas.
        </div>
      `;
      return;
    }

    el.classList.innerHTML = classes.map(cls => {
      const participantCount = classParticipantCount(cls.id);
      const attemptCount = classAttemptCount(cls.id);
      const status = cls.is_active ? "Aktif" : "Ditutup";

      return `
        <article class="class-card ${cls.is_active ? "" : "inactive"}">
          <div class="class-card-main">
            <div>
              <span class="class-status ${cls.is_active ? "active" : ""}">${status}</span>
              <h4>${escapeHtml(cls.class_name)}</h4>
              <div class="class-code-row">
                <code>${escapeHtml(cls.class_code)}</code>
                <button class="class-mini-action" type="button"
                  data-copy-code="${escapeHtml(cls.class_code)}">Salin kode</button>
              </div>
            </div>

            <div class="class-stats">
              <span><strong>${participantCount}</strong> peserta</span>
              <span><strong>${attemptCount}</strong> pengerjaan</span>
            </div>
          </div>

          <div class="class-card-actions">
            <button class="button button-outline class-share-btn" type="button"
              data-share-class="${escapeHtml(cls.class_code)}">
              Salin tautan peserta
            </button>
            <button class="text-button" type="button"
              data-toggle-class="${escapeHtml(cls.id)}"
              data-next-active="${cls.is_active ? "false" : "true"}">
              ${cls.is_active ? "Tutup kelas" : "Aktifkan kelas"}
            </button>
          </div>
        </article>
      `;
    ...
    el.classList.querySelectorAll("[data-copy-code]").forEach(btn => {
      btn.addEventListener("click", () => copyText(btn.dataset.copyCode, btn));
    });

    el.classList.querySelectorAll("[data-share-class]").forEach(btn => {
      btn.addEventListener("click", () =>
        copyText(shareClassUrl(btn.dataset.shareClass), btn)
      );
    });

    el.classList.querySelectorAll("[data-toggle-class]").forEach(btn => {
      btn.addEventListener("click", async () => {
        const classId = btn.dataset.toggleClass;
        const nextActive = btn.dataset.nextActive === "true";
        const action = nextActive ? "mengaktifkan" : "menutup";

        if (!confirm(`Anda yakin ingin ${action} kelas ini?`)) return;

        try {
          await STORE.updateClassStatus(classId, nextActive);
          await loadDashboard();
        } catch (error) {
          console.error(error);
          alert("Status kelas belum berhasil diubah.");
        }
      });
    });
  }
  ...
  function renderFilters() {
    ...
    const legacyNames = latestRows
      .filter(r => !r.class_id)
      .map(r => String(r.cohort_name || "").trim())
      .filter(Boolean);

    const cohorts = [...new Set([...classNames, ...legacyNames])]
      .sort((a,b) => a.localeCompare(b, "id"));

    el.cohortFilter.innerHTML = `
      <option value="all">Semua kelas</option>
      ${cohorts.map(name => `<option value="${escapeHtml(name)}">${escapeHtml(name)}</option>`).join("")}
      <option value="__none__">Legacy / tanpa kelas</option>
    `;
  ...
  function openParticipant(participantId) {
    ...
    const registeredParticipant = registeredParticipants.find(p => p.id === participantId) || null;
    const participantClass = registeredParticipant
      ? classes.find(c => c.id === registeredParticipant.class_id) || null
      : null;
    ...
    el.modalBody.innerHTML = `
      ...
      <section class="participant-section participant-admin">
        ...
        ${registeredParticipant ? `
          <div class="participant-identity-admin">
            <div class="identity-admin-row">
              <span>Kelas</span>
              <strong>${escapeHtml(participantClass?.class_name || cohort)}</strong>
            </div>
            <div class="identity-admin-row">
              <span>Kode kelas</span>
              <strong>${escapeHtml(participantClass?.class_code || "—")}</strong>
            </div>

            <button id="resetParticipantCodeBtn" class="button button-outline" type="button">
              Reset Kode Peserta
            </button>

            <div id="resetCodeResult" class="reset-code-result" hidden>
              <span>KODE PESERTA BARU</span>
              <strong id="resetCodeValue">—</strong>
              <small>Kode lama langsung tidak berlaku.</small>
            </div>
          </div>
        ` : `
          <div class="participant-admin-grid">
            <label>
              <span>Kelas / batch legacy</span>
              <input id="participantCohortEdit" type="text" maxlength="100"
                value="${escapeHtml(cohort === "—" ? "" : cohort)}"
                placeholder="Nama kelas/batch">
            </label>
    ...
    const resetCodeBtn = document.getElementById("resetParticipantCodeBtn");
    ...
    saveCohortBtn?.addEventListener("click", async () => {
      ...
      try {
        ...
        adminMessage.textContent = "Kelas/batch legacy berhasil diperbarui.";
        await loadDashboard();
      } catch (error) {
        console.error(error);
    ...
    resetCodeBtn?.addEventListener("click", async () => {
      const ok = confirm(
        "Buat Kode Peserta baru? Kode lama langsung tidak dapat digunakan lagi."
      );
      if (!ok) return;

      adminMessage.textContent = "Membuat kode baru…";

      try {
        const newCode = await STORE.resetParticipantCode(participantId);
        resetCodeValue.textContent = newCode;
        resetCodeResult.hidden = false;
        adminMessage.textContent =
          "Kode baru berhasil dibuat. Sampaikan hanya kepada peserta yang bersangkutan.";
      } catch (error) {
        console.error(error);
        adminMessage.textContent = "Kode peserta belum berhasil direset.";
      }
    });
    ...
    deleteParticipantBtn?.addEventListener("click", async () => {
      ...
      const ok = confirm(
        `Hapus SELURUH identitas dan riwayat evaluasi ${name}? Tindakan ini tidak dapat dibatalkan.`
      );
      if (!ok) return;
  ...
  el.createClassForm?.addEventListener("submit", async event => {
    event.preventDefault();

    const className = String(el.newClassName.value || "").trim();
    const classCode = STORE.normalizeCode(el.newClassCode.value);

    if (className.length < 2 || classCode.length < 4) {
      el.classFormMessage.textContent =
        "Isi nama kelas dan kode kelas minimal 4 karakter.";
      return;
    }

    el.classFormMessage.textContent = "Membuat kelas…";

    try {
      await STORE.createClass(className, classCode);
      el.classFormMessage.textContent = "Kelas berhasil dibuat.";
      el.createClassForm.reset();
      await loadDashboard();
    } catch (error) {
      console.error(error);
      const msg = String(error?.message || "");
      el.classFormMessage.textContent = /duplicate|unique/i.test(msg)
        ? "Kode kelas sudah digunakan. Gunakan kode lain."
        : "Kelas belum berhasil dibuat.";
    }
  });
