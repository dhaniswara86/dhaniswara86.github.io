(() => {
  ...
  const state = {
    ...
    resident: '', notIndonesianResident: '', dgt: '', properPurpose: '', notConduit: '', singaporeDualResidenceOutcome: '', usDualResidenceIncorporation: '', usLobPublicTrade: '', usLobQualifiedOwnership: '', usLobBaseErosion: '', usLobPrincipalPurpose: '',
  ...
  document.querySelectorAll('[data-country]').forEach(b => b.addEventListener('click', e => {
    const nextCountry = e.currentTarget.dataset.country;
    const countryChanged = Boolean(state.country && state.country !== nextCountry);
    if (countryChanged) resetStep3EligibilityForCountryChange();
  ...
  function bindStep3SegmentGroup(group) {
    if (!group || group.dataset.step3Bound === 'true') return;
    group.dataset.step3Bound = 'true';
    group.querySelectorAll('button').forEach(btn => btn.addEventListener('click', () => {
      ...
      if (group.dataset.field === 'noSpecialRelationship') {
        ...
        const showArmLength = state.noSpecialRelationship === 'no';
        setStrictVisibility(row, showArmLength);
        if (!showArmLength) state.armLengthAmount = '';
      }
      if (group.dataset.field === 'notIndonesianResident') renderCountryResidenceRule();
      if (group.dataset.field?.startsWith('usLob')) syncUsLobAlternatives();
      checkStep3();
    }));
  }

  document.querySelectorAll('[data-step="3"] .segmented').forEach(bindStep3SegmentGroup);
  ...
  function configureStep3() {
    ...
    renderCountryResidenceRule();
    syncUsLobGroup();
    syncUsLobAlternatives();
    checkStep3();
  }

  const COUNTRY_RESIDENCE_RULES = {
    Singapore: {
      field: 'singaporeDualResidenceOutcome', className: 'sg-only-review',
      title: 'Dual-resident entity Indonesia–Singapura: hasil MAP Article 4',
      help: 'Jika entitas merupakan resident Indonesia dan Singapura, Article 4(3) P3B Indonesia–Singapura mengharuskan competent authorities menyelesaikan status tersebut melalui mutual agreement. Pilih hasil yang telah ditetapkan atau status prosesnya.',
      options: [['singapore','Singapura'],['indonesia','Indonesia'],['noagreement','Belum sepakat'],['unknown','Belum tahu']]
    },
    Japan: {
      field: 'dualResidenceMap', className: 'japan-only-review',
      title: 'Dual-resident entity: MAP menetapkan Jepang sebagai treaty residence',
      help: 'Untuk non-individual yang resident di Indonesia dan Jepang, MLI Article 4 mengharuskan competent authorities menentukan treaty residence melalui mutual agreement. Tanpa agreement, relief/exemption treaty tidak tersedia.',
      options: [['yes','Ya'],['no','Tidak'],['unknown','Belum tahu']]
    },
    'United States': {
      field: 'usDualResidenceIncorporation', className: 'us-only-review',
      title: 'Dual-resident company: tempat organisasi / incorporation',
      help: 'Article 4(4) P3B Indonesia–Amerika Serikat menentukan perusahaan yang resident di kedua negara dianggap resident treaty di negara tempat perusahaan tersebut organized atau incorporated.',
      options: [['us','Amerika Serikat'],['indonesia','Indonesia'],['unknown','Belum tahu']]
    },
    Netherlands: {
      field: 'dualResidenceMap', className: 'nl-only-review',
      title: 'Dual-resident entity: MAP menetapkan Belanda sebagai treaty residence',
      help: 'MLI Article 4 mengganti place-of-effective-management rule. Competent authorities harus menentukan treaty residence dengan mempertimbangkan place of effective management, tempat incorporation/constitution, dan faktor relevan lain. Tanpa agreement, relief/exemption P3B tidak tersedia.',
      options: [['yes','Ya'],['no','Tidak'],['unknown','Belum tahu']]
    },
    Australia: {
      field: 'dualResidenceMap', className: 'au-only-review',
      title: 'Dual-resident entity: MAP menetapkan Australia sebagai treaty residence',
      help: 'MLI Article 4 mengganti place-of-effective-management rule. Competent authorities harus menentukan treaty residence dengan mempertimbangkan place of effective management, tempat incorporation/constitution, dan faktor relevan lain. Tanpa agreement, relief/exemption P3B tidak tersedia.',
      options: [['yes','Ya'],['no','Tidak'],['unknown','Belum tahu']]
    }
  };
  const countryResidenceStateFields = ['singaporeDualResidenceOutcome','dualResidenceMap','usDualResidenceIncorporation'];
  const step3CountryBoundFields = [...baseEligibilityFields, ...beneficialOwnerFields, ...passiveRelationshipFields, ...substanceFields, 'armLengthAmount', ...countryResidenceStateFields, 'usLobPublicTrade','usLobQualifiedOwnership','usLobBaseErosion','usLobPrincipalPurpose'];

  function setStrictVisibility(el, show) {
    if (!el) return;
    el.hidden = !show;
    ...
    el.querySelectorAll('button,input,select,textarea').forEach(control => { control.disabled = !show; });
  }

  function clearCountryResidenceState(exceptField = '') {
    countryResidenceStateFields.forEach(field => {
      if (field !== exceptField) state[field] = '';
    });
  }

  function resetStep3EligibilityForCountryChange() {
    step3CountryBoundFields.forEach(field => { state[field] = ''; });
    const panel = document.querySelector('[data-step="3"]');
    panel?.querySelectorAll('.segmented button.is-selected').forEach(btn => btn.classList.remove('is-selected'));
    const slot = document.getElementById('countryResidenceSlot');
    if (slot) slot.innerHTML = '';
    setStrictVisibility(slot, false);
    const arm = document.getElementById('armLengthRow');
    setStrictVisibility(arm, false);
    const usGroup = document.getElementById('usLobGroup');
    setStrictVisibility(usGroup, false);
    const toStep4 = document.getElementById('toStep4');
    if (toStep4) toStep4.disabled = true;
  }

  function requiredCountryResidenceField() {
    if (state.recipient !== 'entity' || state.notIndonesianResident !== 'no') return '';
    return COUNTRY_RESIDENCE_RULES[state.country]?.field || '';
  }

  function renderCountryResidenceRule() {
    const slot = document.getElementById('countryResidenceSlot');
    if (!slot) return;
    const rule = COUNTRY_RESIDENCE_RULES[state.country];
    const show = Boolean(rule && state.recipient === 'entity' && state.notIndonesianResident === 'no');
    if (!show) {
      clearCountryResidenceState();
      slot.innerHTML = '';
      slot.removeAttribute('data-rendered-country');
      setStrictVisibility(slot, false);
      return;
    }
    clearCountryResidenceState(rule.field);
    const options = rule.options.map(([value,label]) => `<button type="button" data-value="${escapeHtml(value)}">${escapeHtml(label)}</button>`).join('');
    slot.innerHTML = `<div class="fact-row ${escapeHtml(rule.className)}" data-country-rule="${escapeHtml(state.country)}"><div><strong>${escapeHtml(rule.title)}</strong><small>${escapeHtml(rule.help)}</small></div><div class="segmented segmented-wrap" data-field="${escapeHtml(rule.field)}">${options}</div></div>`;
    slot.setAttribute('data-rendered-country', state.country);
    setStrictVisibility(slot, true);
    const group = slot.querySelector('.segmented');
    bindStep3SegmentGroup(group);
    if (state[rule.field]) {
      group?.querySelector(`[data-value="${state[rule.field]}"]`)?.classList.add('is-selected');
    }
  }
  ...
  function syncUsLobGroup() {
    ...
    setStrictVisibility(group, show);
    if (!show) {
  ...
  function usLobFactsReady() {
    ...
    if (publicRoute || ownershipRoute || purposeRoute) return true;
    if (!state.usLobPublicTrade || !state.usLobQualifiedOwnership || !state.usLobPrincipalPurpose) return false;
    if (state.usLobQualifiedOwnership !== 'no' && !state.usLobBaseErosion) return false;
    return true;
  }

  function syncUsLobAlternatives() {
    if (state.country !== 'United States' || state.recipient !== 'entity' || isPersonalIncome()) return;
    const ownershipRow = document.getElementById('usLobOwnershipRow');
    const baseErosionRow = document.getElementById('usLobBaseErosionRow');
    const purposeRow = document.getElementById('usLobPurposeRow');
    const publicPass = state.usLobPublicTrade === 'yes';
    const ownershipPass = state.usLobQualifiedOwnership === 'yes' && state.usLobBaseErosion === 'yes';
    const purposePass = state.usLobPrincipalPurpose === 'yes';

    setStrictVisibility(ownershipRow, !(publicPass || purposePass));
    setStrictVisibility(baseErosionRow, !(publicPass || purposePass || !['yes','unknown'].includes(state.usLobQualifiedOwnership)));
    setStrictVisibility(purposeRow, !(publicPass || ownershipPass));

    if (publicPass) {
      state.usLobQualifiedOwnership = '';
      state.usLobBaseErosion = '';
      state.usLobPrincipalPurpose = '';
      [ownershipRow, baseErosionRow, purposeRow].forEach(row => row?.querySelectorAll('button').forEach(b => b.classList.remove('is-selected')));
    } else if (purposePass) {
      state.usLobQualifiedOwnership = '';
      state.usLobBaseErosion = '';
      [ownershipRow, baseErosionRow].forEach(row => row?.querySelectorAll('button').forEach(b => b.classList.remove('is-selected')));
    } else if (ownershipPass) {
      state.usLobPrincipalPurpose = '';
      purposeRow?.querySelectorAll('button').forEach(b => b.classList.remove('is-selected'));
    } else if (state.usLobQualifiedOwnership === 'no') {
      state.usLobBaseErosion = '';
      baseErosionRow?.querySelectorAll('button').forEach(b => b.classList.remove('is-selected'));
    }
  }
  ...
  function checkStep3() {
    ...
    const countryResidenceField = requiredCountryResidenceField();
    if (countryResidenceField) fields.push(countryResidenceField);
    const usLobRequired = state.country === 'United States' && state.recipient === 'entity' && !isPersonalIncome();
    const answered = fields.every(k => state[k]) && (!usLobRequired || usLobFactsReady());
  ...
  function renderRateFacts() {
    ...
    state.butPkpMode = 'reconcile';
    ['ownershipPct','holdingCondition','holdingPmk365','nlDividendRecipientType','interestCategory','royaltyType','serviceActivity','performedInIndonesia','fixedPlace','prepAuxiliary','serviceThreshold','usTaxYear30','jpGovCoop','antiFragmentationRisk','projectThreshold','projectRole','projectOwnMore30','projectRelatedActivities','projectAggregateThreshold','agentAuthority','agentManufactureProcessing','agentStockDelivery','insurancePE','agentPrincipalRole','independentAgent','agentAlmostWholly','capitalAssetType','capitalShareMarket','capitalPEConnected','capitalPropertyRich','capitalSellerOwnership50','capitalBusinessUseException','capitalReorgException','butRevenue','butOtherIncome','butForceAttractionIncome','butDirectCost','butDepAmort','butHoAdmin','butOtherDeductible','butPositiveAdjustments','butNegativeAdjustments','butLossCarryforward','butTaxableIncome','butPph22Credit','butPph23Credit','butPph25Credit','butPph26Credit','butReinvestment','personalPerformedIndonesia','personalFixedBase','personalPresenceTrigger','employmentInIndonesia','employmentDayCondition','employmentForeignEmployer','employmentNotBornePE','employmentResidenceTaxed','directorBoardCapacity','directorCompanyIndonesia','pensionType','pensionIndonesiaSource','entertainerIndonesia','entertainerThreshold','entertainerException','governmentPaymentType','governmentPayerIndonesia','governmentServicesIndonesia','governmentResidentNational','governmentNotSolePurpose','governmentBusiness','teacherIndonesia','teacherTwoYears','teacherInstitution','teacherPriorResident','teacherPublicInterest','teacherResidenceTaxed','teacherFirstVisit','teacherGovInvitation','usStockSalesContribution','branchPscContract'].forEach(k => state[k] = '');
  ...
  function renderDividendFacts(box, treaty) {
    ...
    const ownershipCard = treaty.dividend.direct != null && treaty.dividend.minOwnership != null
      ? `<div class="rate-card" id="divOwnershipCard"><label><span>Persentase kepemilikan langsung penerima pada perusahaan Indonesia</span><input id="ownershipPct" type="number" min="0" max="100" step="0.01" placeholder="Contoh: 30"></label><p class="rate-help">Dipakai untuk menentukan apakah direct participation rate tersedia menurut treaty ${treaty.label}.</p></div>`
      : `<div class="rate-card"><span class="rate-title">Tarif dividen treaty</span><p class="rate-help">P3B Indonesia–${treaty.label} pada cabang data pilot ini tidak membedakan tarif dividen berdasarkan persentase kepemilikan langsung. Karena itu, persentase ownership tidak ditanyakan.</p></div>`;
    box.innerHTML = `${nlRecipient}${ownershipCard}${holding}`;
  ...
  function renderPersonalFacts(box, treaty) {
    ...
    if (state.income === 'governmentService') {
      ...
      box.innerHTML = `
        ...
        <div class="rate-card"><span class="rate-title">Apakah jasa dilakukan di Indonesia?</span><div class="segmented dynamic-segment" data-rate-field="governmentServicesIndonesia"><button type="button" data-value="yes">Ya, di Indonesia</button><button type="button" data-value="no">Tidak, di negara partner</button><button type="button" data-value="unknown">Belum tahu</button></div></div>
  ...
  function bindDynamicSegments() {
    document.querySelectorAll('.dynamic-segment').forEach(group => group.querySelectorAll('button').forEach(btn => btn.addEventListener('click', () => {
      ...
      if (state.income === 'capitalGain') syncCapitalGainUI(DATA.treaties[state.country]);
      updatePreview(); checkStep4();
    })));
  }
  ...
  function passiveRateFactsReady() {
    if (state.income === 'dividend') {
      ...
      const treaty = DATA.treaties[state.country];
      if (treaty.dividend.direct != null && treaty.dividend.minOwnership != null && state.ownershipPct === '') return false;
  ...
  function evaluatePE() {
    ...
    if (state.fixedPlace === 'yes') {
      ...
      if (state.prepAuxiliary === 'yes') {
        ...
        else notes.push('Fixed place diklaim preparatory/auxiliary dan tidak ada anti-fragmentation trigger berdasarkan jawaban pengguna; tetap simpan bukti fungsi aktual sebagai dokumentasi.');
      }
        ...
        if (covered) {
          if (state.serviceThreshold === 'yes') {
            if (state.country === 'United States') {
              ...
              if (state.jpGovCoop === 'yes') notes.push('Duration threshold terlewati, tetapi Article 5(5) Jepang mengecualikan furnishing of services berdasarkan agreement antar-Pemerintah mengenai economic/technical cooperation; duration test tidak memicu PE dan fixed place/agent test tetap diuji.');
          ...
          notes.push(`Untuk ${treaty.label}, ${serviceActivityLabel(state.serviceActivity)} tidak termasuk duration-based service/project PE yang diuji dalam cabang Article 5 ini; keputusan PE ditentukan oleh fixed place dan agent tests yang relevan.`);
        }
      }
    }
    ...
    if (reasons.length) return { status: 'pe', reasons, reviews, notes };
    if (reviews.length) return { status: 'review', reasons, reviews, notes };
    return { status: 'nope', reasons: ['Tidak ada pemicu PE yang teridentifikasi dari fixed place, service/project duration test, dan agent test yang ditanyakan dalam pilot.'], reviews: [], notes };
  }
  ...
  function assessEligibility() {
    ...
    if (state.notIndonesianResident === 'no') {
      ...
      else if (state.country === 'Singapore' && state.recipient === 'entity') {
        if (state.singaporeDualResidenceOutcome === 'singapore') review.push('Entity juga resident Indonesia, tetapi pengguna menyatakan competent authorities melalui MAP Article 4(3) telah menyelesaikan status dual residence dengan hasil Singapura. Dokumen MAP harus diverifikasi.');
        else if (state.singaporeDualResidenceOutcome === 'indonesia') hardFail.push('Dual-resident entity Indonesia–Singapura dinyatakan/ditetapkan sebagai resident Indonesia untuk penerapan treaty pada fakta yang dimasukkan; tarif WPLN treaty Singapura tidak diterapkan otomatis.');
        else if (state.singaporeDualResidenceOutcome === 'noagreement') review.push('Competent authorities Indonesia–Singapura belum/tidak mencapai penyelesaian status dual residence. Protocol P3B memiliki mekanisme khusus atas taxable income dalam keadaan tersebut; pilot tidak memaksakan tarif treaty otomatis dan memerlukan analisis MAP/dokumen terkait.');
        else review.push('Hasil mutual agreement Article 4(3) untuk dual-resident entity Indonesia–Singapura belum diketahui.');
  ...
  function renderServiceResult() {
    ...
    const warnings = [...elig.review, ...pe.reviews, ...(pe.notes || [])];
