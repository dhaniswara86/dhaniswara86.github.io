(() => {
  ...
  function syncUsLobAlternatives() {
    ...
    if (ownershipRow) ownershipRow.hidden = publicPass || purposePass;
    if (baseErosionRow) baseErosionRow.hidden = publicPass || purposePass || !['yes','unknown'].includes(state.usLobQualifiedOwnership);
    if (purposeRow) purposeRow.hidden = publicPass || ownershipPass;
