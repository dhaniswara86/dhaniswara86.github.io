(() => {
  ...
  const state = {
    ...
    serviceThreshold: '', usTaxYear30: '', projectThreshold: '', projectRole: '', projectOwnMore30: '', projectRelatedActivities: '', projectAggregateThreshold: '', agentAuthority: '', agentPrincipalRole: '', independentAgent: '',
    ...
    personalPerformedIndonesia: '', personalFixedBase: '', personalPresenceTrigger: '', employmentInIndonesia: '', employmentDayCondition: '', employmentForeignEmployer: '', employmentNotBornePE: '', employmentResidenceTaxed: '', directorBoardCapacity: '', directorCompanyIndonesia: '',
  ...
  const personalIncomeKeys = ['independentPersonal','employment','directorsFee'];
  ...
  function renderRateFacts() {
    ...
    if (amountLabel) amountLabel.textContent = state.income === 'capitalGain' ? 'Nilai pengalihan / harga jual' : state.income === 'employment' ? 'Nilai bruto remunerasi' : state.income === 'directorsFee' ? 'Nilai bruto directors’ fees' : state.income === 'independentPersonal' ? 'Nilai bruto jasa profesional' : 'Nilai bruto transaksi';
    ...
    if (isPersonalIncome()) {
      if (step4Title) step4Title.textContent = state.income === 'independentPersonal' ? 'Apakah Indonesia memperoleh hak pemajakan atas jasa profesional ini?' : state.income === 'employment' ? 'Apakah employment exemption dalam P3B terpenuhi?' : 'Apakah pembayaran ini benar merupakan directors’ fees menurut P3B?';
      if (step4Copy) step4Copy.textContent = 'Kabayan menyesuaikan pertanyaan presence test, fixed base, employer test, dan directors’ fees article berdasarkan negara treaty yang dipilih.';
  ...
  function personalDomesticTreatment(result) {
    ...
    if (result.status === 'indonesiaMayTax') return {rate:DATA.domestic.rate,label:'PPh Pasal 26 indikatif untuk SPLN'};
  ...
  function syncCapitalGainUI(treaty) {
    ...
    if (needProperty) {
      if (rule === 'singapore50x50') {
        if (propertyTitle) propertyTitle.textContent = 'Apakah >50% nilai saham berasal langsung/tidak langsung dari immovable property di Indonesia?';
        if (propertyHelp) propertyHelp.textContent = 'P3B Singapura Article 13(4) menggunakan threshold nilai >50%, kemudian masih menguji kepemilikan alienator ≥50% dan dua pengecualian.';
  ...
  function renderServiceFacts(box, treaty) {
    ...
    activity?.addEventListener('change', () => {
      if (state.serviceActivity !== activity.value) {
        ['serviceThreshold','usTaxYear30','projectThreshold','projectRole','projectOwnMore30','projectRelatedActivities','projectAggregateThreshold'].forEach(k => state[k] = '');
      }
  ...
  function syncServiceConditional(treaty) {
    ...
    if (isProjectActivity && state.performedInIndonesia === 'yes') {
      let thresholdText = projectRule.wording;
      ...
      if (specialRoleApplies && state.projectRole === 'other-than-main') {
        thresholdText = projectRule.specialRole.wording;
      }
      if (projectHelp) projectHelp.textContent = `${treaty.label}: ${thresholdText}.`;
      if (projectQuestion) projectQuestion.textContent = `Apakah kegiatan ${serviceActivityLabel(state.serviceActivity)} melewati ${specialRoleApplies && state.projectRole === 'other-than-main' ? projectRule.specialRole.threshold : projectRule.threshold}?`;
      ...
      if (aggApplies) {
        ...
        if (aggregateQuestion) aggregateQuestion.textContent = `Setelah periode yang relevan digabung, apakah total durasi melewati ${projectRule.threshold}?`;
      }
    } else {
  ...
  function updateButReconciliation() {
    ...
    if (hasAny && r.fiscalBeforeLoss <= 0) {
      ...
      cls += ' is-review';
      text = 'Ada komponen yang memerlukan review treaty/dokumen: cakupan penghasilan kantor pusat dan/atau alokasi biaya administrasi kantor pusat. Kabayan memasukkan angka sesuai input Anda, tetapi tidak menyatakan komponen tersebut otomatis memenuhi syarat.';
    } else if (hasAny) {
      cls += ' is-ok';
  ...
  function evaluateCapitalGain() {
    ...
    if (asset === 'shares') {
      ...
      if (state.country === 'Singapore') {
        ...
        if (state.capitalPropertyRich === 'unknown') return { status: 'review', article, reason: 'Perlu diketahui apakah >50% nilai saham berasal dari immovable property di Indonesia.' };
        ...
        return { status: 'indonesiaMayTax', article, reason: 'Article 13(4) P3B Singapura terpenuhi: >50% nilai saham berasal dari immovable property Indonesia, alienator memiliki ≥50% total issued shares, dan pengecualian yang diuji tidak dipilih.' };
      }
  ...
  function evaluatePE() {
    ...
    if (state.performedInIndonesia === 'unknown') {
      ...
      if (isProject) {
        const specialRole = projectRule.specialRole?.types?.includes(state.serviceActivity);
        const thresholdWording = specialRole && state.projectRole === 'other-than-main' ? projectRule.specialRole.wording : projectRule.wording;
        ...
        if (state.projectThreshold === 'yes') {
          ...
          if (state.projectOwnMore30 === 'yes') {
            ...
            if (state.projectRelatedActivities === 'yes') {
              if (state.projectAggregateThreshold === 'yes') reasons.push(`Project PE terindikasi setelah aggregation rule MLI (splitting-up of contracts): periode connected activities membuat total melewati ${projectRule.threshold}.`);
              if (state.projectAggregateThreshold === 'unknown') reviews.push(`Periode proyek perlu diagregasi berdasarkan MLI, tetapi belum diketahui apakah totalnya melewati ${projectRule.threshold}.`);
            }
          }
        }
      } else {
  ...
  function renderServiceResult() {
    ...
    if (elig.status === 'eligible' && pe.status === 'pe' && state.butPkpMode === 'reconcile' && numericStateValue('butForceAttractionIncome') > 0) warnings.push('Anda memasukkan penghasilan kantor pusat dari transaksi sejenis/serupa. Pastikan cakupan tersebut memang diperbolehkan oleh P3B yang berlaku dan aturan atribusi laba yang relevan.');
