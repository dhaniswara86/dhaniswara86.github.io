...
const els = {
  ...
  bpmpLoginPanel: $("bpmpLoginPanel"),
  bpmpLoginTin: $("bpmpLoginTin"),
...
function selectFormat(code) {
  ...
  if (els.bpmpLoginPanel) els.bpmpLoginPanel.hidden = code !== "BPMP";
  if (code !== "BPMP" && els.bpmpLoginTin) {
    els.bpmpLoginTin.value = "";
    els.bpmpLoginTin.removeAttribute("aria-invalid");
  }
  if (ot) {
...
function resetFile() {
  ...
  els.fileInput.value = "";
  if (els.bpmpLoginTin) els.bpmpLoginTin.removeAttribute("aria-invalid");
...
function expectedDerivedValues(cfg, normalized, row, rules) {
  ...
  if (cfg.code === "BPMP" && normalized.TaxCertificate !== "ETC") {
    const gross = Number(normalized.Gross);
    if (Number.isFinite(gross) && normalized.StatusTaxExemption) {
      const rate = terRate(gross, normalized.StatusTaxExemption);
      if (rate !== null) expected.Rate = rate;
    }
  }
...
function validateParsed(parsed, cfg, context = {}) {
  ...
  if (cfg.code === "BPMP") {
    validateBPMPHeaderRules(issues, parsed, context);
  }
  ...
  parsed.rows.forEach(row => {
    ...
    if (cfg.code === "BPMP") validateBPMPRowRules(issues, row, normalized, cfg, rules);
...
function validateBPMPHeaderRules(issues, parsed, context) {
  const loginRaw = context?.loginTin ?? "";
  const loginTin = compactTin(loginRaw);
  const excelTin = compactTin(parsed.tin);

  if (!loginTin) {
    issues.push(issue("error", "—", "NPWP login Coretax", "Isi NPWP yang digunakan untuk login Coretax pada Step 2. Template BPMP mensyaratkan NPWP Pemotong sama dengan NPWP login."));
    return;
  }
  if (!/^\d{15,16}$/.test(loginTin)) {
    issues.push(issue("error", "—", "NPWP login Coretax", `NPWP login "${loginRaw}" tidak sesuai pola 15–16 digit yang digunakan template BPMP.`));
    return;
  }
  if (excelTin && loginTin !== excelTin) {
    issues.push(issue("error", "—", "NPWP Pemotong", `NPWP Pemotong pada Excel (${excelTin}) berbeda dengan NPWP login Coretax yang Anda masukkan (${loginTin}). Sesuai validasi template BPMP, keduanya harus sama.`));
  }
}

function validateBPMPRowRules(issues, row, n, cfg, rules) {
...
function validateCurrent() {
  ...
  state.issues = validateParsed(state.parsed, cfg, {loginTin: els.bpmpLoginTin?.value || ""});
...
if (els.bpmpLoginTin) {
  els.bpmpLoginTin.addEventListener("input", () => {
    const compact = compactTin(els.bpmpLoginTin.value);
    els.bpmpLoginTin.toggleAttribute("aria-invalid", !!compact && !/^\d{15,16}$/.test(compact));
    if (state.selected === "BPMP" && state.validated) {
      state.validated = false;
      state.xml = "";
      state.xmlFilename = "";
      els.convertButton.disabled = true;
      els.successSection.hidden = true;
      els.xmlPreviewBox.hidden = true;
      setValidationState("NPWP login berubah · periksa ulang", "neutral");
      updateStepFlow();
    }
  });
}
