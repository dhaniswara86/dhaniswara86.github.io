...
async function loadTemplateManifest() {
  try {
    const response = await fetch(`${TEMPLATE_MANIFEST_PATH}?v=20260816-23`, {cache:"no-store"});
...
const els = {
  ...
  formatPickerTrigger: $("formatPickerTrigger"),
  formatPickerPanel: $("formatPickerPanel"),
  formatPickerBackdrop: $("formatPickerBackdrop"),
  formatPickerClose: $("formatPickerClose"),
  formatPickerSearch: $("formatPickerSearch"),
  formatPickerResults: $("formatPickerResults"),
  formatPickerValue: $("formatPickerValue"),
  formatPickerEyebrow: $("formatPickerEyebrow"),
  formatRequiredNotice: $("formatRequiredNotice"),
  processWrap: $("processWrap"),
  formatPickerCount: $("formatPickerCount"),
  ...
  validationPromptTitle: $("validationPromptTitle"),
  validationPromptText: $("validationPromptText"),
  validationStats: $("validationStats"),
  summaryDetails: $("summaryDetails"),
  step4Hint: $("step4Hint"),
  railStep1: $("railStep1"), railStep2: $("railStep2"), railStep3: $("railStep3"), railStep4: $("railStep4"),
  railConnector1: $("railConnector1"), railConnector2: $("railConnector2"), railConnector3: $("railConnector3")
};
...
const FORMAT_GROUPS = ["PPh Pasal 21/26", "PPh Unifikasi", "e-Faktur / PPN", "SPT Masa PPN", "Dokumen Lain / PPN", "Bea Meterai", "SPT Tahunan PPh Orang Pribadi", "SPT Tahunan PPh Badan"];
...
function renderFormatSelect() {
  ...
  FORMAT_GROUPS.forEach(category => {
  ...
  if (current) els.formatSelect.value = current;
  else els.formatSelect.selectedIndex = -1;
  if (els.formatPickerCount) els.formatPickerCount.textContent = `${FORMAT_ORDER.length} format tersedia`;
  renderFormatPickerResults(els.formatPickerSearch?.value || "");
}

function formatSearchText(f) {
  return `${f.code} ${f.title || ""} ${f.subtitle || ""} ${f.category || ""}`.toLocaleLowerCase("id-ID");
}

function renderFormatPickerResults(query = "") {
  if (!els.formatPickerResults) return;
  const q = String(query || "").trim().toLocaleLowerCase("id-ID");
  els.formatPickerResults.innerHTML = "";
  let visibleCount = 0;

  FORMAT_GROUPS.forEach(category => {
    const matches = FORMAT_ORDER
      .map(code => FORMATS[code])
      .filter(Boolean)
      .filter(f => f.category === category)
      .filter(f => !q || formatSearchText(f).includes(q));

    if (!matches.length) return;
    visibleCount += matches.length;

    const group = document.createElement("section");
    group.className = "kxml-format-picker__group";
    const heading = document.createElement("h3");
    heading.className = "kxml-format-picker__group-title";
    heading.textContent = category;
    group.appendChild(heading);

    const list = document.createElement("div");
    list.className = "kxml-format-picker__list";
    matches.forEach(f => {
      const button = document.createElement("button");
      button.type = "button";
      button.className = "kxml-format-picker__option" + (f.code === state.selected ? " is-selected" : "");
      button.dataset.formatCode = f.code;
      button.setAttribute("aria-current", f.code === state.selected ? "true" : "false");

      const code = document.createElement("span");
      code.className = "kxml-format-picker__code";
      code.textContent = f.code;

      const copy = document.createElement("span");
      copy.className = "kxml-format-picker__option-copy";
      const title = document.createElement("strong");
      title.textContent = f.subtitle || f.title || f.code;
      const meta = document.createElement("small");
      meta.textContent = OFFICIAL_TEMPLATES[f.code]?.version ? `Template ${templateVersionLabel(OFFICIAL_TEMPLATES[f.code])}` : category;
      copy.append(title, meta);

      const mark = document.createElement("span");
      mark.className = "kxml-format-picker__selected-mark";
    ...
    els.formatPickerResults.appendChild(group);
  });

  if (!visibleCount) {
    const empty = document.createElement("div");
    empty.className = "kxml-format-picker__empty";
    const strong = document.createElement("strong");
    strong.textContent = "Format tidak ditemukan";
    const span = document.createElement("span");
    span.textContent = "Coba gunakan kode dokumen atau kata kunci lain.";
    empty.append(strong, span);
    els.formatPickerResults.appendChild(empty);
  }
}

function openFormatPicker() {
  if (!els.formatPickerPanel || !els.formatPickerTrigger) return;
  els.formatPicker?.classList.add("is-open");
  els.formatPickerPanel.classList.add("is-open");
  els.formatPickerBackdrop?.classList.add("is-open");
  els.formatPickerPanel.setAttribute("aria-hidden", "false");
  els.formatPickerBackdrop?.setAttribute("aria-hidden", "false");
  els.formatPickerTrigger.setAttribute("aria-expanded", "true");
  document.body.classList.add("kxml-format-picker-open");
  if (els.formatPickerSearch) {
    els.formatPickerSearch.value = "";
    renderFormatPickerResults("");
    window.setTimeout(() => els.formatPickerSearch.focus(), 30);
  }
}

function closeFormatPicker(returnFocus = false) {
  if (!els.formatPickerPanel || !els.formatPickerTrigger) return;
  els.formatPicker?.classList.remove("is-open");
  els.formatPickerPanel.classList.remove("is-open");
  els.formatPickerBackdrop?.classList.remove("is-open");
  els.formatPickerPanel.setAttribute("aria-hidden", "true");
  els.formatPickerBackdrop?.setAttribute("aria-hidden", "true");
  els.formatPickerTrigger.setAttribute("aria-expanded", "false");
  document.body.classList.remove("kxml-format-picker-open");
  if (!state.selected) setFormatRequiredNotice(true, true);
  if (returnFocus) els.formatPickerTrigger.focus();
}

function setFormatRequiredNotice(show, emphasize = false) {
  if (!els.formatRequiredNotice) return;
  els.formatRequiredNotice.hidden = !show;
  if (show && emphasize) {
    els.formatRequiredNotice.classList.remove("is-attention");
    void els.formatRequiredNotice.offsetWidth;
    els.formatRequiredNotice.classList.add("is-attention");
    window.setTimeout(() => els.formatRequiredNotice?.classList.remove("is-attention"), 420);
  }
}

function selectFormat(code) {
  if (!FORMATS[code]) return;
  const firstSelection = !state.selected;
  state.selected = code;
  setFormatRequiredNotice(false);
  if (els.processWrap) els.processWrap.hidden = false;
  if (els.formatPickerTrigger) els.formatPickerTrigger.classList.remove("is-empty");
  if (els.formatPickerEyebrow) els.formatPickerEyebrow.textContent = "Format dipilih";
  resetFile();
  const f = FORMATS[code];
  const ot = OFFICIAL_TEMPLATES[code];
  if (els.formatSelect) els.formatSelect.value = code;
  if (els.formatPickerValue) els.formatPickerValue.textContent = `${f.code} — ${f.subtitle}`;
  if (els.formatPickerPanel?.classList.contains("is-open")) renderFormatPickerResults(els.formatPickerSearch?.value || "");
...
function setRailState(el, mode) {
  if (!el) return;
  el.classList.remove("is-active","is-complete","is-error","is-locked");
  if (mode) el.classList.add(`is-${mode}`);
}

function setValidationPrompt(mode, title, text) {
  if (!els.validationPrompt) return;
  els.validationPrompt.className = `kxml-validation-prompt is-${mode || "neutral"}`;
  if (els.validationPromptTitle) els.validationPromptTitle.textContent = title;
  if (els.validationPromptText) els.validationPromptText.textContent = text;
}

function updateStepFlow() {
  setStepState(els.step1Card, els.step1State, "complete", "Template tersedia");
  setRailState(els.railStep1, "complete");
  setRailState(els.railConnector1, "complete");
  ...
  if (!state.file) {
    ...
    setRailState(els.railStep2, "active");
    setRailState(els.railStep3, "locked");
    setRailState(els.railStep4, "locked");
    setRailState(els.railConnector2, "locked");
    setRailState(els.railConnector3, "locked");
    if (els.step4Hint) els.step4Hint.textContent = "Unggah dan periksa file terlebih dahulu.";
    return;
  }

  setStepState(els.step2Card, els.step2State, "complete", "File diunggah");
  setRailState(els.railStep2, "complete");
  setRailState(els.railConnector2, "complete");

  if (!state.validated) {
    setStepState(els.step3Card, els.step3State, "active", "Siap diperiksa");
    setStepState(els.step4Card, els.step4State, "locked", "Menunggu pemeriksaan");
    setRailState(els.railStep3, "active");
    setRailState(els.railStep4, "locked");
    setRailState(els.railConnector3, "locked");
    if (els.step4Hint) els.step4Hint.textContent = "Selesaikan Step 3 terlebih dahulu.";
    return;
  }
  ...
  if (errors) {
    ...
    setRailState(els.railStep3, "error");
    setRailState(els.railStep4, "locked");
    setRailState(els.railConnector3, "error");
    if (els.step4Hint) els.step4Hint.textContent = `${errors} error masih harus diperbaiki sebelum XML dapat dibuat.`;
  } else {
    ...
    setRailState(els.railStep3, "complete");
    setRailState(els.railConnector3, "complete");
    setRailState(els.railStep4, state.xml ? "complete" : "active");
    if (els.step4Hint) els.step4Hint.textContent = state.xml ? "XML sudah dibuat dan siap diunduh." : "Data lolos pemeriksaan. XML siap dibuat.";
  }
}
function resetFile() {
  state.workbook = null;
  ...
  if (els.validationStats) els.validationStats.hidden = true;
  if (els.summaryDetails) els.summaryDetails.hidden = true;
  setValidationState("Belum ada file", "neutral");
  setValidationPrompt("neutral", "Belum ada data untuk diperiksa.", "Unggah file pada Step 2 terlebih dahulu.");
  updateStepFlow();
}
...
function renderValidation() {
  ...
  if (els.validationStats) els.validationStats.hidden = !state.validated;
  if (els.summaryDetails) els.summaryDetails.hidden = !state.validated;
  if (errors) {
    setValidationState("Perlu diperbaiki", "error");
    setValidationPrompt("error", `${errors} error ditemukan.`, "Perbaiki data yang ditandai pada hasil pemeriksaan sebelum membuat XML.");
  } else if (warnings) {
    setValidationState("Siap dengan catatan", "warning");
    setValidationPrompt("warning", `Pemeriksaan selesai dengan ${warnings} peringatan.`, "Tidak ada error blocking. Tinjau peringatan sebelum melanjutkan ke Step 4.");
  } else if (state.parsed) {
    setValidationState("Siap dibuat XML", "ok");
    setValidationPrompt("ok", "Pemeriksaan selesai. Tidak ditemukan error.", "Data dapat dilanjutkan ke Step 4 untuk dibuat menjadi XML.");
  } else {
    setValidationState("Belum ada file", "neutral");
    setValidationPrompt("neutral", "Belum ada data untuk diperiksa.", "Unggah file pada Step 2 terlebih dahulu.");
  }
...
async function loadFile(file) {
  ...
  try {
    ...
    setValidationPrompt("ready", "File siap diperiksa.", "Tekan Periksa Data untuk menjalankan validasi khusus template ini.");
    updateStepFlow();
...
if (els.formatPickerTrigger) els.formatPickerTrigger.addEventListener("click", openFormatPicker);
if (els.formatPickerClose) els.formatPickerClose.addEventListener("click", () => closeFormatPicker(true));
if (els.formatPickerBackdrop) els.formatPickerBackdrop.addEventListener("click", () => closeFormatPicker(true));
if (els.formatPickerSearch) els.formatPickerSearch.addEventListener("input", () => renderFormatPickerResults(els.formatPickerSearch.value));
if (els.formatPickerResults) {
  els.formatPickerResults.addEventListener("click", event => {
    const option = event.target.closest("[data-format-code]");
    if (!option) return;
    const code = option.dataset.formatCode;
    if (code && code !== state.selected) selectFormat(code);
    closeFormatPicker(true);
  });
}
document.addEventListener("keydown", event => {
  if (event.key === "Escape" && els.formatPickerPanel?.classList.contains("is-open")) closeFormatPicker(true);
});
...
renderFormatSelect();
setFormatRequiredNotice(!state.selected);
if (els.processWrap) els.processWrap.hidden = true;
loadTemplateManifest().then(() => {
  renderFormatSelect();
  if (state.selected) selectFormat(state.selected);
});
})();
