...
function renderFormatSelect() {
  ...
  const groups = ["PPh Pasal 21/26", "PPh Unifikasi"];
...
function expectedDerivedValues(cfg, normalized, row, rules) {
  ...
  if (cfg.code === "BP21" && first && normalized.TaxCertificate !== "ETC") {
    // BP21 v.4: Deemed dan Tarif mengikuti referensi kode objek pajak, kecuali
    // jika menggunakan fasilitas perpajakan lainnya (ETC). Pada ETC, template
    // secara eksplisit memperbolehkan nilai yang berbeda dari referensi.
