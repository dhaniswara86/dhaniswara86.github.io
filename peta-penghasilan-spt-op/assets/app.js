(function(){
  ...
  function renderPosition(){
    ...
    if(complete){
      ...
      els.choices.innerHTML=current.choices.map(choice=>`<button class="position-choice${position[current.key]===choice.value?' selected':''}" type="button" data-value="${esc(choice.value)}"><span class="choice-icon"><svg aria-hidden="true"><use href="#i-arrow"></use></svg></span><span><strong>${esc(choice.label)}</strong><small>${esc(choice.note)}</small></span></button>`).join("");
